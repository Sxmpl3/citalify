         ...
         x-data="{
             days: [
                 @foreach($allDays as $d)
                 {
                     ...
                     open:       '{{ $indexed->has($d) ? substr($indexed->get($d)->open_time, 0, 5) : '09:00' }}',
                     close:      '{{ $indexed->has($d) ? substr($indexed->get($d)->close_time, 0, 5) : '19:00' }}',
                     hasBreak:   {{ ($indexed->has($d) && $indexed->get($d)->break_start) ? 'true' : 'false' }},
                     breakStart: '{{ ($indexed->has($d) && $indexed->get($d)->break_start) ? substr($indexed->get($d)->break_start, 0, 5) : '14:00' }}',
                     breakEnd:   '{{ ($indexed->has($d) && $indexed->get($d)->break_end)   ? substr($indexed->get($d)->break_end,   0, 5) : '15:00' }}',
                 },
                 @endforeach
             ],
